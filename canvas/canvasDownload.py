...
##################################################
# DOES NOT WORK
# MIGHT MAKE IT WORK but makes lots of assumptions
#
# Downloaded from https://github.com/skuhl/autograder/blob/master/canvas.py
# on 20 October 2017
# Author: Scott Kuhl
###################################################
#
...
class canvas():
    ...
    def __init__(self, token=None, api=None, courseId=None):
        canvasTokenFile = os.path.expanduser(CANVASTOKEN)
        if token and api:
    ...
    def commentOnSubmission(self, courseId, assignmentId, studentId, comment):
        ...
        if type(assignmentId) != int or type(studentId) != int:
            print("Can't comment on a submission without a assignment ID and a student ID.")
            exit(1)
...
#                return
    ...
    def removeGit(self, subdirName):
        ...
        for dirpath, dnames, _ in os.walk(subdirName):
            for d in dnames:
                fullpath = os.path.join(dirpath, d)
    ...
    def removeAutograder(self, subdirName):
        ...
        for dirpath, _, fnames in os.walk(subdirName):
            for f in fnames:
                fullpath = os.path.join(dirpath, f)
                if os.path.isfile(fullpath):
    ...
    def extractFile(self, filename, subdir, newSubdir=False):
